...
def edicao(request):
    ...
    # Handle POST request using service
    if request.method == "POST":
        return setup_service.handle_prescription_edit_request(request, setup, ModeloFormulario, escolhas, medicamentos, processo_id)
...
def _handle_prescription_create_post(request, setup, ModeloFormulario, escolhas, medicamentos, paciente_existe):
    """Handle POST request for prescription creation with simplified control flow."""
    ...
    try:
        ...
        return json_response.duplicate_process()
    except Exception as e:
...
def _get_patient_context_data(request, setup):
    ...
    # Add prescription-specific data
