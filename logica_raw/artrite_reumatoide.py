...
class Artrite_Reumatoide(GeradorPDF):
    ...
    def primeira_vez(self, dados_lme_base, dados_condicionais, path_lme_base):
        ...
        issue_exams = final_data['exames']
        base_files = [lme_base_path, PATH_AR_CONSENT]
        medication = selecionar_med_consentimento(medication, final_data)
        if (medication in meds_relatorio_ar):
            base_files = [lme_base_path, PATH_AR_CONSENT, REPORT_AR_PATH]
        if issue_exams:
            if (medication in med_ar_grupo_1):
                final_data['exames_solicitados'] = final_data.pop('exames_ar_grupo_1')
                files = adicionar_exames(base_files, final_data)
            else:
                final_data['exames_solicitados'] = final_data.pop('exames_ar_grupo_2')
                files = adicionar_exames(base_files, final_data)
        else:
            files = preencher_formularios(base_files, final_data)
        pdf = gerar_pdf_final(files, PATH_PDF_FINAL)
